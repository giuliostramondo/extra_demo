...
#        return -1
    ...
    def get_block_after_pragma(self,pragma_name):
        pragma_id=self.find_polymem_pragma(pragma_name)
        main_index=self.find_main_func()
        return self.ast.ext[main_index].body.block_items[pragma_id+1]
    ...
    def replace_block_after_pragma(self, pragma_name, new_block):
        index_pragma= self.find_polymem_pragma(pragma_name)
        func_index=self.find_main_func()
        self.ast.ext[func_index].body.block_items[index_pragma+1:index_pragma+1]=new_block       
