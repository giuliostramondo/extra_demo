...
def plot_array_to_pdf(image, item_to_access,title,coverage=[],coverage_id=-1,type=0):
    if coverage == [] or coverage_id != -1:
        for to_access in item_to_access:
            image[to_access[0]][to_access[1]]+=5
    ...
    #plt.gca().set_yticks([y - 0.5 for y in plt.gca().get_yticks()][1:], minor='true')
    #plt.gca().set_title("")
    #plt.gcf().set_size_inches(len(image[0])/2, len(image)/2)
    #plt.grid(which='minor',color='b', linestyle='-', linewidth=2)
    #plt.savefig("trace.pdf", bbox_inches='tight', dpi=50)
...
if __name__ == "__main__":
    ...
    'writePDFTrace':plot_array_to_pdf,
	#'conv': convertFile,
    }
